...
//This activity should be started after taking a picture, //intent extras below:
    ...
    val model = Cropnet.newInstance(baseContext) //TODO change this to implement the specific model, this is a placeholder to test the app
            ...
            when(tag)
            {
                Config.PATH_TAG -> {
                    try{
                        ...
                        Log.e("ImageTensorLoading","Error in decoding and loadinf file:\n ${e.printStackTrace()}")
                    }
                }
                Config.BYTE_TAG -> {
                    val byteImg:ByteArray = extras.getByteArray(Config.BYTE_TAG)!!
                    val bitmap:Bitmap = BitmapFactory.decodeByteArray(byteImg,0,byteImg.size)
                    tensorImage.load(bitmap)
                }
                else -> {
